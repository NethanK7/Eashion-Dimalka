<div class="grid grid-cols-1 gap-8 mb-8 md:grid-cols-2">
    <div class="relative group">
        ...
        <div class="relative">
            <select wire:model.live="province" name="province" id="province-select" class="w-full px-1 py-4 text-base text-gray-900 transition-all duration-300 bg-white border-b-2 border-gray-100 appearance-none cursor-pointer focus:outline-none focus:border-black">
                <option value="">Select Province</option>
                @foreach($provinces as $prov)
                    <option value="{{ $prov }}" wire:key="prov-{{ $prov }}">{{ $prov }}</option>
                @endforeach
            </select>
            <div class="absolute -translate-y-1/2 pointer-events-none right-2 top-1/2 opacity-40">
